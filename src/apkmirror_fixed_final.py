...
def get_download_link(version: str, app_name: str, config: dict, arch: str = None) -> str:
    ...
    def _extract_variant_link(row):
        ...
        for link in links:
            ...
            # Ignore publisher/developer landing pages such as /apk/x-corp/.
            if '/apk/' in href:
                tail = href.split('/apk/', 1)[1].strip('/')
                if len([part for part in tail.split('/') if part]) < 3:
                    continue
                score += 10
            if 'download' in text:
                score += 5
        ...
        for link in found_soup.find_all('a', href=True):
            ...
            for _ in range(12):
                ...
                if has_arch and (has_dpi or has_android) and has_package_type:
                    # FIRST complete parent is the variant card.
                    # Do NOT keep climbing: larger ancestors contain other
                    # variants and merge their architecture/DPI metadata.
    ...
    # User target DPI. 522 is the device target.
    TARGET_DPI = 522

    def _dpi_options(variant):
        """Return normalized DPI options from one exact variant."""
        options = []
        ...
        for low, high in variant["dpi_ranges"]:
            ...
            options.append({
                ...
                "high": high,
                "width": high - low,
                "contains": low <= TARGET_DPI <= high,
                "above": low > TARGET_DPI,
                "distance": (
                    0 if low <= TARGET_DPI <= high
                    else low - TARGET_DPI if low > TARGET_DPI
                    else TARGET_DPI - high
                ),
            })
        ...
        for dpi in variant["dpis"]:
            ...
            options.append({
                ...
                "high": dpi,
                "width": 0,
                "contains": dpi == TARGET_DPI,
                "above": dpi > TARGET_DPI,
                "distance": abs(dpi - TARGET_DPI),
            })

        return options

    def _select_dpi_candidates(candidates):
        """Apply the exact 522-DPI rules to candidate variants."""
        normal = [v for v in candidates if not v["is_nodpi"]]

        # Rule 1: narrowest range/value containing 522.
        containing = []
        for variant in normal:
            for opt in _dpi_options(variant):
                if opt["contains"]:
                    containing.append((opt["width"], variant))
        if containing:
            min_width = min(width for width, _ in containing)
            return [v for width, v in containing if width == min_width]

        # Rule 2: options containing a number/range ABOVE 522.
        # Among all such options choose the variants whose selected option
        # has the smallest file size, regardless of range vs single DPI.
        above = []
        for variant in normal:
            for opt in _dpi_options(variant):
                if opt["above"]:
                    above.append((variant["file_size"], opt["low"], variant))
        if above:
            min_size = min(size for size, _, _ in above)
            return [v for size, _, v in above if size == min_size]
        ...
        # Rule 4: closest DPI number/range to 522; for equal distance,
        # prefer the narrower range. Then normal tie-breaks apply.
        nearest = []
        for variant in normal:
            for opt in _dpi_options(variant):
                nearest.append((opt["distance"], opt["width"], variant))
        if not nearest:
            return candidates

        min_distance = min(distance for distance, _, _ in nearest)
        nearest = [x for x in nearest if x[0] == min_distance]
        min_width = min(width for _, width, _ in nearest)
        return [v for _, width, v in nearest if width == min_width]
    ...
    def _select_for_architecture(architecture):
        ...
        selected = _select_dpi_candidates(candidates)
        return _tie_break(selected)
