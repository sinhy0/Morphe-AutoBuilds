...
def get_download_link(version: str, app_name: str, config: dict, arch: str = None) -> str:
    ...
    def _dpi_options(v):
        return [("range", int(lo), int(hi)) for lo, hi in v["dpi_ranges"]] + \
               [("single", int(d), int(d)) for d in v["dpis"]]

    def _tie_break(candidates):
        if not candidates:
            return None
        a12 = [v for v in candidates if v["android_version"] == 12]
        if a12:
            candidates = a12
        if len(candidates) > 1:
            size = min(v["file_size"] for v in candidates)
            candidates = [v for v in candidates if v["file_size"] == size]
        return sorted(candidates, key=lambda v: v["index"])[0]

    def _select_for_architecture(architecture):
        candidates = [v for v in variants if architecture in v["architectures"]]
        if not candidates:
            return None

        # 1) Narrowest DPI range/value containing 522.
        hits = []
        for v in candidates:
            for _, lo, hi in _dpi_options(v):
                if lo <= DPI_TARGET <= hi:
                    hits.append((hi - lo, v))
        if hits:
            width = min(w for w, _ in hits)
            return _tie_break([v for w, v in hits if w == width])

        # 2) Any option extending above 522 -> smallest file.
        above = [
            v for v in candidates
            if any(hi > DPI_TARGET for _, lo, hi in _dpi_options(v))
        ]
        if above:
            return _tie_break(above)
        ...
        # 4) Closest value/range to 522, then narrowest.
        scored = []
        for v in candidates:
            opts = _dpi_options(v)
            if not opts:
                continue
            best = min(
                (0 if lo <= DPI_TARGET <= hi else min(abs(lo-DPI_TARGET), abs(hi-DPI_TARGET)),
                 hi-lo)
                for _, lo, hi in opts
            )
            scored.append((best, v))
        if scored:
            score = min(x[0] for x in scored)
            return _tie_break([v for x, v in scored if x == score])
...
def get_latest_version(app_name: str, config: dict) -> str:
    """Return the newest version listed by APKMirror, including beta/alpha."""
    try:
        main_url = f"{base_url}/apk/{config['org']}/{config['name']}/"
        response = _cf_get(main_url)
        if response.status_code == 200:
            ...
            for row in soup.find_all("div", class_="appRow"):
                ...
                if not title or not title.a:
                    continue
                txt = title.a.get_text(" ", strip=True)
                m = version_pattern.search(txt)
                if m:
                    v = m.group(0)
                    nums = tuple(int(x) for x in re.findall(r'\d+', v.split('-')[0]))
