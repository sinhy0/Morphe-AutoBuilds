...
def get_download_link(version: str, app_name: str, config: dict, arch: str = None) -> str:
    ...
    def _extract_variant_link(row):
        """Return the actual variant/release link, not publisher/navigation links."""
        candidates = []
        for order, link in enumerate(row.find_all('a', href=True)):
            ...
            # Reject generic publisher/category/navigation URLs such as
            # /apk/x-corp/ which caused the previous "Variant Page" bug.
            if re.fullmatch(r'/apk/[^/]+/?', path, re.I):
                continue
            if path.lower() in {'/apk/', '/faq/', '/contact/'}:
                continue

            score = 0
            if '/download/' in href.lower() or 'download' in href.lower():
                score += 100
            ...
            if text:
                score += 2
            candidates.append((score, -order, link))

        if not candidates:
            return None
        candidates.sort(key=lambda x: (x[0], x[1]), reverse=True)
        return candidates[0][2].get('href')
    ...
    if not variants:
        ...
        for link in found_soup.find_all('a', href=True):
            ...
            # Use the SMALLEST ancestor that contains the complete metadata.
            # Never keep climbing after a valid variant container is found.
            container = link
            best_container = None
            ...
            for _ in range(12):
                ...
                parent_text = _clean_text(parent.get_text(' ', strip=True)).lower()
                ...
                has_android = bool(re.search(r'\bandroid\s+\d', parent_text))
                has_package_type = bool(re.search(r'\b(?:apk|bundle|aab)\b', parent_text))
    ...
    def _dpi_options(variant):
        """Return (kind, low, high, distance, width) options for one variant."""
        options = []
        for low, high in variant["dpi_ranges"]:
            options.append(("range", low, high, 0 if low <= DPI_TARGET <= high else min(abs(low-DPI_TARGET), abs(high-DPI_TARGET)), high-low))
        for dpi in variant["dpis"]:
            options.append(("single", dpi, dpi, abs(dpi-DPI_TARGET), 0))
        return options

    def _tie_break(candidates):
        """Tie-break exactly as requested: Android 12, then size, then first."""
        if not candidates:
            return None

        # Prefer the highest minimum Android version not exceeding 12.
        eligible = [
            v for v in candidates
            if v["android_version"] is not None and v["android_version"] <= 12
        ]
        if eligible:
            best_android = max(v["android_version"] for v in eligible)
            candidates = [v for v in eligible if v["android_version"] == best_android]
        elif any(v["android_version"] is not None for v in candidates):
            known = [v for v in candidates if v["android_version"] is not None]
            closest = min(abs(v["android_version"] - 12) for v in known)
            candidates = [v for v in known if abs(v["android_version"] - 12) == closest]

        if len(candidates) > 1:
            size = min(v["file_size"] for v in candidates)
            candidates = [v for v in candidates if v["file_size"] == size]

        return min(candidates, key=lambda v: v["index"])
    ...
    def _select_for_architecture(architecture):
        ...
        logging.info(f"Found {len(candidates)} variants for architecture {architecture}")

        normal = [v for v in candidates if not v["is_nodpi"]]

        # Rule 1: narrowest RANGE containing 522. If a standalone value is
        # exactly 522, it is also considered a zero-width range.
        containing = []
        for v in normal:
            for kind, low, high, distance, width in _dpi_options(v):
                if low <= DPI_TARGET <= high:
                    containing.append((width, v))
                    break
        if containing:
            min_width = min(width for width, _ in containing)
            best = [v for width, v in containing if width == min_width]
            return _tie_break(best)

        # Rule 2: no option contains 522. Look for options whose DPI/range
        # has a value ABOVE 522. Among those choose the smallest file.
        above = []
        for v in normal:
            if any(high > DPI_TARGET for _, low, high, _, _ in _dpi_options(v)):
                above.append(v)
        if above:
            size = min(v["file_size"] for v in above)
            best = [v for v in above if v["file_size"] == size]
            return _tie_break(best)
        ...
        # Rule 4: nearest DPI number/range endpoint to 522; for a range,
        # use its closest endpoint, and prefer the narrower range on a tie.
        nearest = []
        for v in normal:
            opts = _dpi_options(v)
            if not opts:
                continue
            distance = min(o[3] for o in opts)
            widths = [o[4] for o in opts if o[3] == distance]
            nearest.append((distance, min(widths), v))

        if nearest:
            best_distance = min(x[0] for x in nearest)
            nearest = [x for x in nearest if x[0] == best_distance]
            best_width = min(x[1] for x in nearest)
            best = [x[2] for x in nearest if x[1] == best_width]
            return _tie_break(best)
    ...
    if not selected_variant:
        logging.info(f"No universal variant exists for {app_name} {version}; trying noarch/nodpi")
        selected_variant = _select_for_architecture("noarch")
        if not selected_variant:
            nodpi_variants = [v for v in variants if v["is_nodpi"]]
            selected_variant = _tie_break(nodpi_variants)
        if selected_variant:
            selected_arch = "noarch/nodpi"
