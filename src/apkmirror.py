...
def get_download_link(version: str, app_name: str, config: dict, arch: str = None) -> str:
    ...
    def _extract_variant(row, index, exact_href=None):
        ...
        # Prefer the exact metadata encoded in the APKMirror variant URL.
        # This prevents a large parent DOM container from mixing metadata from
        # multiple variants.
        variant_href = exact_href or _extract_variant_link(row)
        architectures = set()
        url_dpis = []
        url_nodpi = False
        ...
        if variant_href:
            try:
                ...
                marker = "variant-"
                pos = variant_href.find(marker)
                if pos >= 0:
                    payload = unquote(variant_href[pos + len(marker):].rstrip("/"))
                    data = json.loads(payload)

                    for arch in data.get("arches_slug", []):
                        arch = str(arch).lower()
                        if arch in {"arm64-v8a", "armeabi-v7a", "x86_64", "x86",
                                    "universal", "noarch"}:
                            architectures.add(arch)

                    for dpi in data.get("dpis_slug", []):
                        dpi = str(dpi).lower()
                        if dpi == "nodpi":
                            url_nodpi = True
                        elif dpi.endswith("dpi") and dpi[:-3].isdigit():
                            url_dpis.append(int(dpi[:-3]))
                        elif dpi.isdigit():
                            url_dpis.append(int(dpi))
            except Exception:
                pass
        ...
        if url_dpis:
            dpis = sorted(set(url_dpis), reverse=True)
            dpi_ranges = []
        elif url_nodpi:
            dpis = []
            dpi_ranges = []
        ...
        is_nodpi = url_nodpi or bool(re.search(r'\bnodpi\b', lower))

        android_version = _extract_android_version(text)
        file_size = _extract_file_size(text)
        href = exact_href or _extract_variant_link(row)
    ...
    if not variants:
        ...
        for link in found_soup.find_all('a', href=True):
            ...
            variant_href = variant['href']
            if variant_href in seen_links:
                continue

            seen_links.add(variant_href)
            variants.append(variant)
    ...
    def _dpi_options(variant):
        ...
        for low, high in variant["dpi_ranges"]:
            options.append(("range", int(low), int(high), int(high) - int(low)))
        for dpi in variant["dpis"]:
            options.append(("single", int(dpi), int(dpi), 0))
        return options

    def _contains_target(variant):
        return any(low <= TARGET_DPI <= high for _, low, high, _ in _dpi_options(variant))

    def _has_number_above_target(variant):
        return any(low > TARGET_DPI or high > TARGET_DPI for _, low, high, _ in _dpi_options(variant))
    ...
    def _tie_break(candidates):
        ...
        # Exact tie-break requested: Android 12 first, then lower minimum
        # Android versions; if still tied, smallest file; then page order.
        android_12_or_lower = [
            v for v in candidates
            if v["android_version"] is not None and v["android_version"] <= 12
        ]
        if android_12_or_lower:
            best_android = max(v["android_version"] for v in android_12_or_lower)
            candidates = [v for v in android_12_or_lower if v["android_version"] == best_android]
        else:
            # If Android 12 or lower is unavailable, use the lowest known
            # minimum rather than preferring a newer requirement.
            known = [v for v in candidates if v["android_version"] is not None]
            if known:
                best_android = min(v["android_version"] for v in known)
                candidates = [v for v in known if v["android_version"] == best_android]

        if len(candidates) > 1:
            smallest = min(v["file_size"] for v in candidates)
            candidates = [v for v in candidates if v["file_size"] == smallest]
    ...
    def _select_for_architecture(architecture):
        ...
        logging.info(
            f"Found {len(candidates)} variants for architecture {architecture}"
        )

        # 1) Narrowest DPI range containing 522. A single DPI is treated as
        # a zero-width range only when it actually equals 522.
        containing = []
        for variant in candidates:
            widths = [
                width for kind, low, high, width in _dpi_options(variant)
                if low <= TARGET_DPI <= high
            ]
            if widths:
                containing.append((min(widths), variant))

        if containing:
            narrowest = min(width for width, _ in containing)
            return _tie_break([
                variant for width, variant in containing
                if width == narrowest
            ])

        # 2) No DPI contains 522: keep every variant having a DPI/range with
        # a number above 522, then choose the smallest file regardless of
        # whether the option is a range or a single DPI.
        higher = [v for v in candidates if _has_number_above_target(v)]
        if higher:
            smallest = min(v["file_size"] for v in higher)
            return _tie_break([v for v in higher if v["file_size"] == smallest])

        # 3) No higher DPI: use nodpi if available.
        nodpi = [v for v in candidates if v["is_nodpi"]]
        if nodpi:
            logging.info(f"No DPI above {TARGET_DPI} for {architecture}; using nodpi")
            return _tie_break(nodpi)

        # 4) No nodpi: choose the closest DPI number/range to 522, then
        # smallest file. This is the final safety net.
        distances = [(_closest_distance(v), v) for v in candidates]
        best_distance = min(distance for distance, _ in distances)
        closest = [v for distance, v in distances if distance == best_distance]
        smallest = min(v["file_size"] for v in closest)
        return _tie_break([v for v in closest if v["file_size"] == smallest])

    # Architecture priority: arm64-v8a -> universal -> noarch.
    selected_variant = _select_for_architecture("arm64-v8a")
    selected_arch = "arm64-v8a" if selected_variant else None

    if not selected_variant:
        logging.info(f"No arm64-v8a variant exists for {app_name} {version}; trying universal")
        selected_variant = _select_for_architecture("universal")
        if selected_variant:
            selected_arch = "universal"

    if not selected_variant:
        logging.info(f"No universal variant exists for {app_name} {version}; trying noarch")
        selected_variant = _select_for_architecture("noarch")
        if selected_variant:
            selected_arch = "noarch"

    if not selected_variant:
        logging.error(f"No APK/BUNDLE variant found for {app_name} {version}")
        return None
    ...
    selected_type = "APK" if selected_variant["is_apk"] else "BUNDLE"
    ...
    if selected_variant["is_nodpi"]:
        ...
        ranges = containing or selected_variant["dpi_ranges"]
        selected_range = min(ranges, key=lambda r: (r[1] - r[0], r[0]))
        selected_dpi = f"{selected_range[0]}-{selected_range[1]}dpi"
    elif selected_variant["dpis"]:
        # For multi-DPI variants report the DPI nearest to the target.
        selected_dpi_value = min(
            selected_variant["dpis"],
            key=lambda d: abs(d - TARGET_DPI)
        )
        selected_dpi = f"{selected_dpi_value}dpi"
    else:
    ...
    logging.info(f"✓ Variant row: {selected_variant['text'][:500]}")
